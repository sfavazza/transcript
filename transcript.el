...
particular `1' to look forward and `-1' to look backward."
  ...
  ;; Generate a customized doc-string for each function generated by this macro
...
Warn the user if no further %s lines can be found." prev-or-next loi-string loi-string)))
  ...
  `(defun ,name ()
     ...
     (let ((pnt)                           ; symbol where point will be stored
       ...
       (save-excursion
         (beginning-of-line)
         ;; In emacs only functions to detect a property change are available, hence the next line
         ;; must be first analyzed to check for the loi.
